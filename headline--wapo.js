...
puppeteer
  ...
  .then(function(page) {
    return page.goto(url, { timeout: 90000 }).then(function() {
      ...
      return page.waitForSelector('.headline > h2 > a > span', '.headline > h2 > a', { timeout: 20000 });
    })
    .then(function() {
      return page.content();
    });
  ...
  .then(function(html) {
    ...
    const articleTitle = $('.headline > h2 > a > span');
    const articleUrl = $('.headline > h2 > a');
    let ms = new Date();
    ...
    fs.writeFile('headlines-world/headline--wapo.json', jsonString, function(err){
